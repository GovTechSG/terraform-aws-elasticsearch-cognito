...
|------|-------------|------|---------|:--------:|
| additional\_cidr\_allow\_443 | CIDR to allow port 443 communication from | `list` | `[]` | no |
...
| dedicated\_master\_type | ES instance type to be used for dedicated masters (default same as instance\_type) | `string` | `"false"` | no |
...
| log\_pusher\_iam\_roles | List of IAM users or role ARNs from which to permit PUT/POST requests meant to send logs to elasticsearch | `list(string)` | `[]` | no |
| management\_iam\_roles | List of IAM role ARNs from which to permit management traffic (default ['\*']).  Note that a client must match both the IP address and the IAM role patterns in order to be permitted access. | `list(string)` | <pre>[<br>  "*"<br>]</pre> | no |
...
| project\_name | n/a | `string` | `"es"` | no |
| security\_group\_ids | List of security groups to apply to the elasticsearch cluster | `list` | `[]` | no |
...
| use\_prefix | Flag indicating whether or not to use the domain\_prefix. Default: true | `bool` | `true` | no |
...
| app\_iam\_access\_key\_id | The access key ID for log pusher |
| app\_iam\_access\_key\_secret | The access key secret for log pusher |
| app\_iam\_user\_arn | The ARN assigned by AWS for log pusher user |
| app\_iam\_user\_name | ES log pusher user's name |
